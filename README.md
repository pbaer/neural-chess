...
- **Browser weights**: the web-tool hero (`D-a10-t2`) is **distilled** from our own human-trained `v3-37M` teacher — no engine signal at any point (see [Distillation](#distillation--the-browser-hero)). Its small Model Capsule under `viz/public/weights/` **is** committed and deployed to GitHub Pages so the web app runs — but because it too derives in part from TWIC (personal-use) data, it is provided **for use within the web tool only, with no separate redistribution/reuse grant** (the MIT code license does not extend to it). The bulk training data and the large GPU model weights remain unpublished / personal-use.
