// MoveCandidates — pick-mode picker. Shows the model's top-K legal moves ranked
// by policy probability; the most strongly predicted move is the most prominent
// and less likely ones fade out. Clicking a row plays that move as the model's.
// The board draws matching arrows (see Board.tsx) so the choice is also spatial.
...
export function MoveCandidates({ store, candidates, onHover }: MoveCandidatesProps) {
  ...
  return (
    <div className="candidates">
      <div className="panel-title">Model's top moves — pick one to play</div>
          ...
          return (
            <li key={c.uci}>
              <button
                ...
                style={{ opacity }}
                onClick={() => store.getState().chooseModelMove(i)}
                onMouseEnter={() => onHover(c.uci)}
                onFocus={() => onHover(c.uci)}
                onBlur={() => onHover(null)}
                title={`${c.san} — ${(c.prob * 100).toFixed(1)}% · click to play`}
              >
