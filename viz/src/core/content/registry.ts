...
export const CONTENT: Record<string, ContentCard> = {
  ...
  tokenize: {
    ...
    why: 'The attention step that follows treats its inputs as an unordered set: without an address, the token for e4 and the token for h1 would look identical. These position codes are how the model keeps track of the layout, and it learns the board’s geometry into them.',
  },
  block: {
  ...
  value_head: {
    ...
    why: 'Alongside the move probabilities, a single score for how good the position is helps judge the situation. That judgment is learned purely from how human games turned out.',
  },
  ...
  residual: {
    ...
    why: 'Keeping a straight-through path means information from early layers survives to the end, and the network can be made deep without the training signal fading: each block makes a small, safe edit.',
  },
};
