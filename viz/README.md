...
The tool is **architecture-version-neutral**: it never reads a PyTorch checkpoint directly. A Python exporter derives a versioned, self-describing **Model Capsule** — `capsule.json` (a manifest incl. the model config and an architecture `graph` of typed stages) + `weights.bin` — and the tool renders whatever architecture the Model Capsule declares. "v3"/"v3.1" appears only in a model's `arch` metadata and its id; nothing in the tool's code is bound to a specific version.

> **Status: BUILT, RUNNING & DEPLOYED (small tweaks going forward).** Milestones **M1 (parity engine) → M2 (play loop + board) → M3 (visualization telescope)** are complete and have been iterated extensively, plus an in-browser MCTS engine, value-adaptive play, a move-assistant, a calibrated Elo strength estimate, and mobile polish. `tsc --noEmit` is clean and the full test suite passes (hundreds of tests, including the PyTorch forward-parity assertions — identical argmax + value error < 1e-4 vs the reference model). Live at [pbaer.github.io/neural-chess](https://pbaer.github.io/neural-chess/) (GitHub Pages, auto-deploys on `viz/**` pushes to `master`). Run locally: `npm install` then `npm run dev` in `viz/`, open `http://localhost:5173/neural-chess/`. The original design + milestone roadmap are preserved (as a historical record) in [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md).
