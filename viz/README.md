...
> **Status: BUILT, RUNNING & DEPLOYED (small tweaks going forward).** Milestones **M1 (parity engine) → M2 (play loop + board) → M3 (visualization telescope)** are complete and have been iterated extensively, plus an in-browser MCTS engine, value-adaptive play, a move-assistant, a calibrated Elo strength estimate, and mobile polish. `tsc --noEmit` is clean and the full test suite passes (734 tests, including the PyTorch forward-parity assertions — identical argmax + value error < 1e-4 vs the reference model). Live at [pbaer.github.io/neural-chess](https://pbaer.github.io/neural-chess/) (GitHub Pages, auto-deploys on `viz/**` pushes to `master`). Run locally: `npm install` then `npm run dev` in `viz/`, open `http://localhost:5173/neural-chess/`. The original design + milestone roadmap are preserved (as a historical record) in [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md).
